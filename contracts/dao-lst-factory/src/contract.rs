...
fn update_scaling_factor(deps: DepsMut, _env: Env, _info: MessageInfo) -> ContractResult {
    let config = CONFIG.load(deps.storage)?;

    let state: StateResponse = deps.querier.query_wasm_smart(config.hub, &HubQueryMsg::State {})?;
    let exchange_rate = state.exchange_rate;

    let scaling_factors = if config.scale_first {
        get_factors(exchange_rate.numerator(), exchange_rate.denominator(), config.decimals)?
    } else {
        get_factors(exchange_rate.denominator(), exchange_rate.numerator(), config.decimals)?
    };

    let factors = scaling_factors.iter().map(|a| a.to_string()).collect::<Vec<String>>().join(",");
    ...
    Ok(
        Response::new()
            ...
            .add_attribute("factors", factors), // .add_message(msg)
    )
}
...
mod tests {
    ...
    fn test_scaling_factors() {
        ...
        let factors = get_factors(exchange_rate.numerator(), exchange_rate.denominator(), Some(9))
        ...
        // cutoff after 9th position
        let exchange_rate = Decimal::from_str("1.192342342456").unwrap();
        let factors = get_factors(exchange_rate.numerator(), exchange_rate.denominator(), Some(2))
            .expect("should be set");
        assert_eq!(factors, vec![119, 100]);

        // cutoff after 9th position
        let exchange_rate = Decimal::from_str("1.192342342456").unwrap();
        let factors = get_factors(exchange_rate.numerator(), exchange_rate.denominator(), Some(1))
            .expect("should be set");
        ...
        assert_eq!(factors.to_string(), "out of range integral type conversion attempted");
