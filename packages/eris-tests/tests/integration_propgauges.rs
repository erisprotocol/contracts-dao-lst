...
fn vote() -> StdResult<()> {
    ...
    helper.mint_utoken(router_ref, "user1", 100_000000);
    helper.hub_bond(router_ref, "user1", 100_000000, helper.base.utoken_denom()).unwrap();
    ...
    result.assert_attribute("wasm", attr("end", "4")).unwrap();
    ...
    helper.prop_vote(router_ref, "user2", 10, cosmwasm_std::VoteOption::Yes).unwrap();
