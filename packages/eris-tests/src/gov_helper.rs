...
use cosmwasm_std::{coin, coins, Addr, Delegation, FullDelegation, StdResult, Uint128, VoteOption};
...
impl EscrowHelper {
    pub fn init(router_ref: &mut CustomApp) -> Self {
    ...
    pub fn mint_token(
        ...
        router_ref
            .sudo(cw_multi_test::SudoMsg::Bank(cw_multi_test::BankSudo::Mint {
                ...
                amount: vec![coin(amount, token.clone())],
            }))
            .unwrap();

        router_ref
            .send_tokens(Addr::unchecked("bank"), Addr::unchecked(to), &coins(amount, token))
            .unwrap();
    }
    ...
    pub fn cw3_execute_sender(
        &self,
        router_ref: &mut CustomApp,
        execute: manta_cw3::msg::ExecuteMsg,
        sender: impl Into<String>,
    ) -> Result<AppResponse> {
        router_ref.execute_contract(
            Addr::unchecked(sender),
            self.base.cw3.get_address(),
            &execute,
            &[],
        )
    }

    pub fn cw3_create_prop(
        &self,
        router_ref: &mut CustomApp,
        sender: impl Into<String>,
    ) -> Result<AppResponse> {
        router_ref.execute_contract(
            Addr::unchecked(sender),
            self.base.cw3.get_address(),
            &manta_cw3::msg::ExecuteMsg::Propose {
    ...
    pub fn cw3_create_20_props(&self, router_ref: &mut CustomApp, sender: String) -> Result<()> {
        for _ in 1..20 {
            self.cw3_create_prop(router_ref, sender.clone())?;
        }
        Ok(())
    }

    pub fn cw4_execute_sender(
        &self,
        router_ref: &mut CustomApp,
        execute: manta_stake::msg::ExecuteMsg,
        sender: impl Into<String>,
    ) -> Result<AppResponse> {
        router_ref.execute_contract(
            Addr::unchecked(sender),
        ...
        router_ref
            .execute(
                ...
                cosmwasm_std::CosmosMsg::Bank(cosmwasm_std::BankMsg::Send {
                    to_address: fake_addr,
                    amount: vec![coin(amount.u128(), ustake_addr.to_string())],
                }),
            )
            .unwrap();
    }
    ...
    //             denom: self.base.steak_token.get_address_string(),
    //         },
    //     )
    // }
    ...
    pub fn arb_unbond(
        ...
        let lp = config.lp_token.denom;

        let msg = eris::arb_vault::ExecuteMsg::Unbond {
            immediate,
        };
